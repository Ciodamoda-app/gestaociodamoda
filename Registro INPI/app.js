...
// ==========================================
const firebaseConfig = window.AppConfig?.firebaseConfig || { apiKey: "SUA_API_KEY" };

// Leitura segura do AppConfig via dot-path com fallback
const cfg = (path, fallback = '') => {
    const keys = path.split('.');
    let val = window.AppConfig;
...
const ViewTemplates = {
    dashb: () => `
        <div class="page-header" style="align-items: flex-end;">
            <div>
                <h2 class="page-title">${cfg('dictionary.dashboardLabel', 'Painel Executivo Financeiro')}</h2>
    ...
    `,
    clientes:     () => generateListView(cfg('dictionary.clientsLabel',      'Clientes'),           'Gerencie sua carteira, contatos B2B e endereços.', 'clientes'),
    fornecedores: () => generateListView(cfg('dictionary.suppliersLabel',    'Fornecedores'),        'Gerencie as fábricas e seus representantes parceiros.', 'fornecedores'),
    produtos:     () => generateListView(cfg('dictionary.productsLabel',     'Produtos em Estoque'), 'Cadastro de referências, SKUs unitários e cores.', 'produtos'),
    transacoes:   () => generateListView(cfg('dictionary.transactionsLabel', 'Módulo de Transações'),'Registros corporativos de Compra (Investimento/Entrada) e Venda (Faturamento/Saída).', 'transacoes'),
    admin: () => `
        <div class="page-header">
            <div>
...
document.addEventListener("DOMContentLoaded", () => {
    // Injeção do White-Label via Configuração
    if (window.AppConfig) {
        const appName = window.AppConfig.appName || "Sistema ERP";
        const elHead = document.getElementById('app-title-head');
        const elLogin = document.getElementById('app-login-title');
        const elBrand = document.getElementById('app-brand-name');
        
        if (elHead) elHead.innerText = `${appName} - Painel Executivo`;
        if (elLogin) elLogin.innerText = appName;
        if (elBrand) elBrand.innerText = appName;
        
        if (window.AppConfig.theme?.primaryColor) {
            document.documentElement.style.setProperty('--c-brand', window.AppConfig.theme.primaryColor);
        }

        // Wire logoUrl: substitui o ícone padrão do sidebar por imagem customizada
        const logoUrl = cfg('theme.logoUrl');
        if (logoUrl) {
            const brandIcon = document.querySelector('.brand-icon');
            if (brandIcon) brandIcon.innerHTML =
                `<img src="${logoUrl}" alt="${appName}" style="width:28px;height:28px;object-fit:contain;">`;
        }

        // Wire dictionary: atualiza labels dos nav links do sidebar
        const navMap = {
            'dashb':        cfg('dictionary.dashboardLabel'),
            'clientes':     cfg('dictionary.clientsLabel'),
            'fornecedores': cfg('dictionary.suppliersLabel'),
            'produtos':     cfg('dictionary.productsLabel'),
            'transacoes':   cfg('dictionary.transactionsLabel'),
        };
        Object.entries(navMap).forEach(([route, label]) => {
            if (!label) return;
        ...
        // Wire modules: feature toggles — oculta módulos desativados no sidebar
        if (cfg('modules.enableSuppliers') === false) {
            document.querySelector('[data-route="fornecedores"]')?.closest('li')?.style.setProperty('display', 'none');
        }
    }
...
window.DB_Core = {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (!parsed.version || !parsed.collections) {
                        showToast(`Arquivo inválido: não parece um backup do ${window.AppConfig?.appName || 'sistema'}.`, 'error');
                        return;
                    }
    ...
    async exportBackup() {
        const cols = ['clientes', 'fornecedores', 'produtos', 'transacoes', 'usuarios_permitidos'];
        const backup = { version: '1.0', exportedAt: new Date().toISOString(), appId: window.AppConfig?.appName || 'coreerp', collections: {} };
        toggleGlobalLoader(true);
        try {
            ...
            a.href = url;
            const prefix = (window.AppConfig?.appName || 'coreerp').toLowerCase().replace(/\s+/g, '');
            a.download = `${prefix}_backup_${new Date().toISOString().slice(0, 10)}.json`;
            a.click();
        ...
        if (entityKey === 'clientes') {
            ...
            formHTML = `
                ...
                <div class="form-group">
                    ...
                    <input type="email" id="ipt-email" ${isEditingId} required class="input-field" placeholder="gestor@empresa.com">
                </div>
                <div class="form-row">
                    <div>
    ...
    initTransactionForm() {
        ...
        // Feature toggle: oculta opção Orçamento se módulo desativado
        if (cfg('modules.enableQuotes') === false) {
            document.querySelector('input[name="tipoOp"][value="orcamento"]')?.closest('.radio-option')?.remove();
        }

        // Feature toggle: oculta carrinho multi-item se módulo desativado
        if (cfg('modules.enableCart') === false) {
    ...
    generateTransactionPDF(data) {
        ...
        pdf.text(window.AppConfig?.appName || 'Sistema ERP', margin, y);
        pdf.setFontSize(9);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(100, 116, 139);
        pdf.text(cfg('dictionary.pdfSubtitle', 'Sistema de Gestão Comercial'), margin, y + 5);
        ...
        pdf.text(`${cfg('appName', 'Sistema ERP')} - ${cfg('dictionary.pdfSubtitle', 'Sistema de Gestão Comercial')} | Documento gerado automaticamente`, pageWidth / 2, 290, { align: 'center' });
